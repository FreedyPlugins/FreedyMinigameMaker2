...
@Processable(alias = "cos")
public class MathCos implements Process {
    ...
    public String run(MiniGame miniGame, ProcUnit procUnit) {
        try {
            if (frontBrace != null) {
                ...
                return FastMath.cosQuick(sum) + frontBrace.getLastProc().run(miniGame, procUnit);
            } else {
                String value = process.run(miniGame, procUnit);
                double sum = Double.parseDouble(value);
                return String.valueOf(FastMath.cosQuick(sum));
            }
