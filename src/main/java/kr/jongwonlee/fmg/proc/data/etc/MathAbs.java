...
@Processable(alias = "abs")
public class MathAbs implements Process {
    ...
    public String run(MiniGame miniGame, ProcUnit procUnit) {
        try {
            if (frontBrace != null) {
                ...
                return FastMath.abs(sum) + frontBrace.getLastProc().run(miniGame, procUnit);
            } else {
                String value = process.run(miniGame, procUnit);
                double sum = Double.parseDouble(value);
                return String.valueOf(FastMath.abs(sum));
            }
