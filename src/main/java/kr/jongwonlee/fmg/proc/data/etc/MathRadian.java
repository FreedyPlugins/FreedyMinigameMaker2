...
@Processable(alias = "radian")
public class MathRadian implements Process {
    ...
    public String run(MiniGame miniGame, ProcUnit procUnit) {
        try {
            if (frontBrace != null) {
                ...
                return FastMath.toRadians(sum) + frontBrace.getLastProc().run(miniGame, procUnit);
            } else {
                String value = process.run(miniGame, procUnit);
                double sum = Double.parseDouble(value);
                return String.valueOf(FastMath.toRadians(sum));
            }
